        ...
        for batch_idx, (data, target, target_orgin, ids) in enumerate(tbar):
            ...
            for i in range(prediction.shape[0]):
                ...
                if np.sum(prediction_post[0, ...]) < 1e-4:
                    ...
                    hd_OC = binary.hd95(np.asarray(prediction_post[0, ...], dtype=bool),
                                        np.asarray(target_numpy[i, 0, ...], dtype=bool))
                    asd_OC = binary.asd(np.asarray(prediction_post[0, ...], dtype=bool),
                                        np.asarray(target_numpy[i, 0, ...], dtype=bool))
                if np.sum(prediction_post[1, ...]) < 1e-4:
                    hd_OD = 100
                    asd_OD = 100
                else:
                    hd_OD = binary.hd95(np.asarray(prediction_post[1, ...], dtype=bool),
                                        np.asarray(target_numpy[i, 1, ...], dtype=bool))

                    asd_OD = binary.asd(np.asarray(prediction_post[1, ...], dtype=bool),
                                        np.asarray(target_numpy[i, 1, ...], dtype=bool))
