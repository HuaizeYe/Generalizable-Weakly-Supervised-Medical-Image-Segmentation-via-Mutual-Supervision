...
def parse_args():
    ...
    parser.add_argument('--test_prediction_save_path', type=str, default='./resultsram', help='Path root for test image and mask')
    ...
    with torch.no_grad():
        for file_name in tbar:
            ...
            for ii in range(int(np.floor(image.shape[0] // args.batch_size))):
                ...
                # print(file_name,vol.shape)
                # for i in range(8):
                #     print(torch.sum(vol[i]))
                # sys.exit(0)
            ...
            hd = binary.hd95(np.asarray(processed_pred_y, dtype=bool),
                                np.asarray(mask, dtype=bool))
            asd = binary.asd(np.asarray(processed_pred_y, dtype=bool),
                                np.asarray(mask, dtype=bool))
            # print(file_name,processed_pred_y.shape,np.sum(processed_pred_y),np.sum(mask))
            # print(dice_coeff,hd,asd)
            count = 0
