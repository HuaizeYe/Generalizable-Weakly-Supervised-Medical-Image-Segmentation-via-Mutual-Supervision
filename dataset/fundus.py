...
class Fundus_Multi(Dataset):
    ...
    def __getitem__(self, index):
        ...
        with open(os.path.join(self.base_dir, id.split(' ')[0].replace('png', 'txt')), 'r') as fp:
            for line in fp:
                line = line.split(' ')
                bbox = {}
                bbox["class"] = int(line[0])
                bbox["cw"] = float(line[1])
                bbox["ch"] = float(line[2])
                bbox["w"] = float(line[3])
                bbox["h"] = float(line[4])
                boxinfo.append(bbox)
            boxinfo.reverse()
        ...
        if self.is_freq:
            ...
            mask = mask.transpose(2, 0, 1)
            mask = torch.from_numpy(np.array(mask)).float()
            return img, img_freq, mask, boxinfo
        else:
            return sample['img'], sample['mask'], boxinfo


class Fundus_Multi3(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.split == 'test':
            ...
            if self.transform:
                sample = self.transform(sample)
            return sample['img'], sample['mask'], mask, boxinfo, id
        ...
        if self.transform:
            sample = self.transform(sample)
        return sample['img'], sample['mask'], boxinfo, id


class Fundus_Multi4(Dataset):
    def __init__(self, domain_idx_list=None, base_dir=None, split='train', num=None, transform=None):
        self.transform = transform
        self.base_dir = base_dir
        self.num = num
    ...
    def __getitem__(self, index):
        ...
        img = Image.open(os.path.join(self.base_dir, id.split(' ')[0]))
        ori_image = img
        ori_image = ori_image.resize((256, 256), Image.BILINEAR)
        ori_image = np.array(ori_image).astype(np.float32)
        ...
        if self.transform:
            sample = self.transform(sample)
        return sample['img'], sample['mask'], boxinfo, id, ori_image
