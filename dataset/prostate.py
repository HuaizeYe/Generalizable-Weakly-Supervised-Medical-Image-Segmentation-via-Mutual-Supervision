...
class Prostate_Multi4(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.split == 'test':
            ...
            sample = {'img': img, 'mask': mask}
            box = open(os.path.join(self.base_dir, id.replace('npy', 'txt').replace('image', 'mask')))
            bbox = list(map(int, box.readline().split(' ')))
            ...
            return img, mask, id.split('/')[-1], bbox, box_mask
            ...
            if self.is_freq:
                ...
                return img, img_freq, box_mask, bbox,box_mask
                ...
                if 'onehot_label' in sample.keys():
                    ...
                    onehot_label = torch.from_numpy(onehot_label).long()
                    return img, mask.long(), onehot_label
                return img, mask.long(), bbox, box_mask


class Prostate_Multi(Dataset):
    def __init__(self, domain_idx_list=None, base_dir=None, split='train', num=None, test_domain_idx=None,transform=None):
        self.base_dir = base_dir
        self.num = num
        self.domain_name = ['Domain1', 'Domain2', 'Domain3', 'Domain4', 'Domain5', 'Domain6']
        self.domain_idx_list = domain_idx_list
...
class Prostate_Pseudo(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.split == 'test':
            mask = np.load(
                os.path.join(self.base_dir.replace('prostate', 'prostate_pseudo_' + str(self.test_domain_idx)),
                             id.replace('image', 'mask')))
            sample = {'img': img, 'mask': mask}
...
class Prostate_Box(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img = np.load(os.path.join(self.base_dir, id))
        cur_domain_name = id.split('/')[0]
        mask = open(os.path.join(self.base_dir, id.replace('npy', 'txt').replace('image', 'mask')))
        bbox = list(map(int, mask.readline().split(' ')))
        bbox = [bbox[0], bbox[1], bbox[0] + bbox[2], bbox[1] + bbox[3]]
...
class Prostate_Multi2(Dataset):
    ...
    def __getitem__(self, index):
        ...
        bbox = [bbox[0], bbox[1], bbox[0] + bbox[2], bbox[1] + bbox[3]]
        mask = np.load(os.path.join(self.base_dir, id.replace('image', 'mask')))
        path = os.path.join(self.base_dir, id.replace('image', 'mask'))
        sample = {'img': img, 'mask': mask}
        img = sample['img']
        mask = sample['mask']
        img = img.transpose(2, 0, 1)
        img = torch.from_numpy(img).float()
        mask = torch.from_numpy(mask).long()
        return img, mask, bbox, path


class Prostate_MakeBox(Dataset):
    def __init__(self, domain_idx_list=None, base_dir=None, split='train', num=None, transform=None):
        self.base_dir = base_dir
        self.num = num
        self.domain_name = ['Domain1', 'Domain2', 'Domain3', 'Domain4', 'Domain5', 'Domain6']
...
class Prostate_Box2(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img = torch.from_numpy(img).float()
        mask = torch.from_numpy(mask).long()
        return img, mask, bbox, id


class Prostate_Pseudo2(Dataset):
    def __init__(self, domain_idx_list=None, domain_unseen=0, base_dir=None, split='train', num=None, transform=None):
        self.base_dir = base_dir
        self.num = num
        self.domain_name = ['Domain1', 'Domain2', 'Domain3', 'Domain4', 'Domain5', 'Domain6']
        self.domain_idx_list = domain_idx_list
        self.split = split
        self.transform = transform
        self.domain_unseen = domain_unseen
        self.id_path = []
        for domain_idx in self.domain_idx_list:
            domain_list = os.listdir(os.path.join(self.base_dir, self.domain_name[domain_idx], 'image'))
            domain_list = [self.domain_name[domain_idx] + '/image/' + item for item in domain_list]
    ...
    def __getitem__(self, index):
        ...
        bbox = [bbox[0], bbox[1], bbox[0] + bbox[2], bbox[1] + bbox[3]]
        mask = np.load(
            os.path.join(self.base_dir.replace('prostate', 'prostate_pseudo_' + str(self.domain_unseen)), id))
        sample = {'img': img, 'mask': mask}
        img = sample['img']
        mask = sample['mask']
        img = img.transpose(2, 0, 1)
